  ...
  return (
    <svg className="pill-caret" width="10" height="6" viewBox="0 0 10 6" fill="none">
      <path
        ...
        stroke="#a1a6c4" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
      />
    </svg>
  );
}

export function TopBar({ address, snapInstalled, network = 'devnet', onNetworkChange, onDisconnect }: Props) {
        ...
        <div className={styles.walletActions}>
          <div ref={networkRef} className={styles.pillAnchor}>
            <button
              ...
              onClick={() => { setNetworkOpen(o => !o); setWalletOpen(false); }}
            >
            ...
            {networkOpen && (
              <NetworkSwitcher
                current={network}
                onSelect={id => { onNetworkChange?.(id as NetworkId); setNetworkOpen(false); }}
              />
            )}
          </div>
          ...
          <div ref={walletRef} className={styles.pillAnchor}>
            ...
            {walletOpen && (
              <WalletMenu
                ...
                onDisconnect={() => { setWalletOpen(false); onDisconnect?.(); }}
              />
            )}
          </div>
        </div>
