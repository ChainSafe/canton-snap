...
export function RegistrationDonePage({
  address, network, onNetworkChange, cantonPartyId, fingerprint, snapInstalled,
  wasAlreadyRegistered, onDashboard, onDisconnect,
}: Props) {
  return (
    <div className="page">
      <AmbientOrb opacity={0.22} size={920} y="58%" />
      <TopBar address={address} snapInstalled={snapInstalled} network={network} onNetworkChange={onNetworkChange} onDisconnect={onDisconnect} />
        ...
        <div className={styles.checkWrap}>
          ...
          <div className={styles.checkCircle}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <path d="M9 18 L15 24 L27 12" stroke="#0a0b14" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
          ...
          <div className={cn('card', styles.partyCard)}>
            <div className="info-row">
              <div className={styles.infoContent}>
                ...
                <p className="info-value">{cantonPartyId.slice(0, 30)}{cantonPartyId.length > 30 ? '…' : ''}</p>
                {cantonPartyId.length > 30 && <p className="info-value-sub">{cantonPartyId.slice(30)}</p>}
              </div>
